...
/**
 * Deletes Firestore event document(s) by the numeric `id` you store on each event.
 * (Your Firestore document id is auto-generated by `addDoc`, so we query by `id`.)
 */
export async function deleteEventByNumericId(eventId: number): Promise<number> {
  const q = query(collection(db, "events"), where("id", "==", eventId));
  const snap = await getDocs(q);
  await Promise.all(snap.docs.map((d) => deleteDoc(d.ref)));
  return snap.size; // how many docs were deleted
}
